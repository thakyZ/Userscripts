...
$(document).ready(() => {
  const crafterDictonary = { CRP: "carpenter", BSM: "blacksmith", ARM: "armorer", GSM: "goldsmith", LTW: "leatherworker", WVR: "weaver", ALC: "alchemist", CUL: "culinarian" };

  const crafterToLong = crafter => crafterDictonary[crafter];
  ...
  const crafterLabel = (crafter, level, stars) => `<small class="label label-success"> <img src="./images/img/${crafterToLong(crafter)}.gif" alt="${crafter}"> ${crafter}：Lv.${level}${genStars(stars)}</small>`;
  ...
  const getObjectData = element => {
    ...
    for (let i = 0; i < labels.length; i++) {
      const test1 = $(labels[i]).text().replaceAll(/(^\s+|\s+$)/gmi, "").split(/[\s：.]/);
      if (/^[A-Z]{3}$/g.test(test1[0])) {
        const testStars = $(labels[i]).html().toString().match(/fa-star/gi) === null ? 0 : $(labels[i]).html().toString().match(/fa-star/gi).length;
