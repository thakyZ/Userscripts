...
// cSpell:ignore steamui, noicon, ascm
...
jQuery(($) => {
  ...
  function setupCSS(css, id) {
    ...
    if (typeof id !== "undefined") {
      ...
      if ($("head > style").filter(x => $(x).text() !== css).length > 0) {
  ...
  function addAllWorkshopItemsToCollection() {
    ...
    function createButtons() {
      if (addAllBtn === undefined && remAllBtn === undefined && progress === undefined && $($("div.collectionAddItemsSection").children()[0]).prop("nodeName") !== "a") {
        ...
        progress = $("<span id=\"add-all-subscribed-collection-manager-progress\" class=\"add-all-subscribed-collection-manager\"></span>");
        $(progress).insertAfter($(remAllBtn));
        $(addAllBtn).on("click", async () => {
          $("#add-all-subscribed-collection-manager-progress").text("<Pending>");
          /** @type {JQueryHTMLElement} */
            ...
            $.ajax({
              ...
              async success(data, textStatus) {
                if (data && textStatus === "success") {
                  ...
                  $("#add-all-subscribed-collection-manager-progress").text(`${Math.ceil((index / totalItems) * 100)}%`);
                  if (index >= totalItems) {
                    $("#add-all-subscribed-collection-manager-progress").text("100% Done!");
                    await sleep(5000);
                    $("#add-all-subscribed-collection-manager-progress").text("");
        ...
        $(remAllBtn).on("click", async () => {
          ...
          for (const [index, item] of Object.entries(collectionItems)) {
            ...
            $("#add-all-subscribed-collection-manager-progress").text(`${Math.floor((index / totalItems2) * 100)}%`);
