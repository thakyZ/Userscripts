...
$(document).ready(function() {
  ...
  const TranslateName = function(name, userId = undefined) {
    if (userId && parseInt(userId) !== NaN && UserNameAlt[parseInt(userId)] !== undefined) {
      return NormalizeNames(UserNameAlt[parseInt(userId)]);
    } else {
  ...
  //$(window).on("load", () => {
  const ChangeAvatarImage = function() {
    const image = $("img[alt=\"User Avatar\"]");
    if (!$(image).prop("complete") || $(image).prop("naturalHeight") === 0) {
      $.ajax({accepts:"data:image/png",cache:false,url:$(image).attr("src"),error:function(jqXHR, textStatus, errorThrown){
  ...
  //});
  if (window.location.href.match(/https:\/\/www\.xivmodarchive\.com\/modid\//gi) !== null) {
    ChangeAvatarImage();
  }
